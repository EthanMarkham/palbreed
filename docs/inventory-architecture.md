...
Supabase adapter must not change Pair Checker, Inventory Planner, Pal Builder,
or the save parser.
...
| --- | --- |
| `inventory_profiles` | `id`, `owner_id`, `name`, `game_version`, `platform`, `world_id`, `slot_id`, `revision`, timestamps |
| `pal_instances` | `id`, `profile_id`, `source_instance_id`, `species_id`, `gender`, `passive_ids`, `location`, `source`, `included`, nickname/level |
...
Use client-generated UUIDs already present in the local records. Upsert imported
Pals on `(profile_id, source_instance_id)`, which makes re-importing a world
idempotent. Re-import preserves each matched Pal's local include/exclude choice,
keeps manual and session-added Pals, and removes stale save-derived instances.
...
3. Upsert save-derived instances by `source_instance_id`; deduplicate manual
   instances by their stable client UUID.
...
4. normalization into stable `OwnedPal` records.
...
- Pair Checker is a forward lookup over the complete loaded 1.0 table.
- Inventory Planner uses breadth-first search, so its continuous-carrier route
  has the minimum number of breedings under owned-instance and gender rules.
- Pal Builder exhaustively searches the finite `(species, gender, required
  passive mask, carried passives)` carrier state space with owned partners. “Cleanest” uses
  estimated expected cakes; probability copy is always labeled as estimated.
