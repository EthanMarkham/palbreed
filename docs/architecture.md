...
Migrating to Next.js is not inherently a bad refactor, but it would add routing, server/runtime, and dependency complexity without solving a current product requirement. Accounts and synced worlds can be added directly to this Vite application. A Next.js migration should occur only when the product needs dynamic server-rendered public pages or a substantial server-only backend-for-frontend.
