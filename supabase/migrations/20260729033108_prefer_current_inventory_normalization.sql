...
alter table public.inventory_profiles
  add column normalization_version integer not null default 1
  constraint inventory_profiles_normalization_version_check
  check (normalization_version between 1 and 32767);
...
begin
  select jsonb_build_object(
    ...
    'profiles', coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
      ...
      'normalizationVersion', profile.normalization_version,
      'pals', coalesce((
  ...
  return coalesce(
    document,
    jsonb_build_object('schemaVersion', 1, 'profiles', '[]'::jsonb)
  );
end;
$$;

drop function public.replace_inventory_profile(
  text, text, text, text, text, text, text, text, text, integer, timestamptz, jsonb
);

create function public.replace_inventory_profile(
  ...
  profile_normalization_version integer default 1
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if profile_normalization_version is null or profile_normalization_version not between 1 and 32767 then raise exception 'The inventory normalization version is invalid.' using errcode = '22023'; end if;
  ...
  -- An already-open older client can omit profile_normalization_version and
  -- therefore sends version 1. Once a physical world has newer normalized
  -- data, ignore that stale write instead of deleting its richer Pal records.
  select profile.*
  into matched_profile
  from public.inventory_profiles as profile
  where profile.user_id = caller_user_id
    and (
      profile.local_profile_id = trim(profile_local_id)
      or (
        profile.platform = profile_platform
        and coalesce(profile.world_id, '') = coalesce(
          nullif(trim(coalesce(profile_world_id, '')), ''),
          ''
        )
        and coalesce(profile.slot_id, '') = coalesce(
          nullif(trim(coalesce(profile_slot_id, '')), ''),
          ''
        )
        and coalesce(profile.account_id, '') = coalesce(
          nullif(trim(coalesce(profile_account_id, '')), ''),
          ''
        )
      )
    )
  order by profile.normalization_version desc, profile.updated_at desc, profile.id desc
  limit 1
  for update;

  if found and matched_profile.normalization_version > profile_normalization_version then
    select count(*)
    into inserted_count
    from public.pal_instances as pal
    where pal.profile_id = matched_profile.id
      and pal.user_id = matched_profile.user_id;

    return jsonb_build_object(
      'profileId', matched_profile.local_profile_id,
      'palCount', inserted_count,
      'revision', matched_profile.revision,
      'normalizationVersion', matched_profile.normalization_version,
      'accepted', false
    );
  end if;
  ...
  values (
    ...
    nullif(trim(coalesce(profile_player_name, '')), ''),
    profile_player_level,
    coalesce(imported_at, now_at),
    now_at,
    now_at,
    1,
  ...
  delete from public.pal_instances as pal
  where pal.profile_id = matched_profile.id;

  for pal_record in
    select * from jsonb_array_elements(pal_records)
  loop
    ...
    canonical_passive_ids := private.canonicalize_inventory_passives(
      array(select jsonb_array_elements_text(passive_json))
    );
    ...
    values (
      ...
      caller_user_id,
      pal_record ->> 'id',
      pal_record ->> 'sourceInstanceId',
      pal_record ->> 'speciesId',
      pal_record ->> 'gender',
      canonical_passive_ids,
      pal_record ->> 'location',
      nullif(pal_record ->> 'palboxSlotIndex', '')::integer,
      nullif(pal_record ->> 'worldId', ''),
      nullif(pal_record ->> 'playerId', ''),
      nullif(pal_record ->> 'nickname', ''),
      nullif(pal_record ->> 'level', '')::integer,
      score_json,
      now_at,
      now_at
    );
...
revoke execute on function public.get_inventory_document()
from public, anon, authenticated;
revoke execute on function public.replace_inventory_profile(
  text, text, text, text, text, text, text, text, text, integer, timestamptz, jsonb, integer
)
from public, anon, authenticated;

grant execute on function public.get_inventory_document()
to authenticated;
grant execute on function public.replace_inventory_profile(
  text, text, text, text, text, text, text, text, text, integer, timestamptz, jsonb, integer
)
to authenticated;
