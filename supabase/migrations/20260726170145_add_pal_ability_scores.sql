...
alter table public.pal_instances
  ...
  check (
    ...
    or (
      ...
      and ability_scores ?& array['hp', 'melee', 'ranged', 'defense']
...
begin
  select jsonb_build_object(
    ...
    'profiles', coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
      'id', profile.local_profile_id, 'owner', jsonb_build_object('kind', 'account', 'id', profile.user_id),
      'name', profile.name, 'gameVersion', profile.game_version, 'platform', profile.platform,
      'worldId', profile.world_id, 'slotId', profile.slot_id, 'accountId', profile.account_id,
      'playerId', profile.player_id, 'playerName', profile.player_name, 'playerLevel', profile.player_level,
      'importedAt', profile.imported_at, 'createdAt', profile.created_at, 'updatedAt', profile.updated_at,
      'revision', profile.revision,
      'pals', coalesce((
        select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
          'id', pal.local_pal_id, 'sourceInstanceId', pal.source_instance_id,
          'speciesId', pal.species_id, 'gender', pal.gender, 'passiveIds', pal.passive_ids,
          'location', pal.location, 'worldId', pal.world_id, 'playerId', pal.player_id,
          'nickname', pal.nickname, 'level', pal.level, 'abilityScores', pal.ability_scores
