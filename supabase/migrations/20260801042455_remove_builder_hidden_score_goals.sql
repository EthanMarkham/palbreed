...
drop function public.record_builder_search(text, text[], text, integer, text, integer, integer, integer);
drop function public.list_recent_builder_searches(text, integer);
drop function public.claim_recent_builder_searches(text);

alter table public.builder_search_history
  drop constraint if exists builder_search_history_minimum_hp_iv_check,
  drop constraint if exists builder_search_history_minimum_attack_iv_check,
  drop constraint if exists builder_search_history_minimum_defense_iv_check,
  drop column if exists minimum_hp_iv,
  drop column if exists minimum_attack_iv,
  drop column if exists minimum_defense_iv;
...
create function public.claim_recent_builder_searches(
...
revoke execute on function public.record_builder_search(text, text[], text, integer, text)
...
grant execute on function public.record_builder_search(text, text[], text, integer, text)
