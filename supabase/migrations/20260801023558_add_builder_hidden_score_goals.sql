...
alter table public.builder_search_history
  add column minimum_hp_iv smallint,
  add column minimum_attack_iv smallint,
  add column minimum_defense_iv smallint,
  add constraint builder_search_history_minimum_hp_iv_check
    check (minimum_hp_iv is null or minimum_hp_iv between 1 and 100),
  add constraint builder_search_history_minimum_attack_iv_check
    check (minimum_attack_iv is null or minimum_attack_iv between 1 and 100),
  add constraint builder_search_history_minimum_defense_iv_check
    check (minimum_defense_iv is null or minimum_defense_iv between 1 and 100);
...
create function public.record_builder_search(
  ...
  search_minimum_hp_iv integer default null,
  search_minimum_attack_iv integer default null,
  search_minimum_defense_iv integer default null
)
returns void
language plpgsql
security definer
...
declare
  ...
  canonical_minimum_hp_iv integer := search_minimum_hp_iv;
  canonical_minimum_attack_iv integer := search_minimum_attack_iv;
  canonical_minimum_defense_iv integer := search_minimum_defense_iv;
...
begin
  ...
  if search_minimum_hp_iv is not null and search_minimum_hp_iv not between 1 and 100 then
    raise exception 'Minimum HP IV must be between one and one hundred.' using errcode = '22023';
  end if;
  if search_minimum_attack_iv is not null and search_minimum_attack_iv not between 1 and 100 then
    raise exception 'Minimum Attack IV must be between one and one hundred.' using errcode = '22023';
  end if;
  if search_minimum_defense_iv is not null and search_minimum_defense_iv not between 1 and 100 then
    raise exception 'Minimum Defense IV must be between one and one hundred.' using errcode = '22023';
  end if;
  ...
  if caller_user_id is not null then
    insert into public.builder_search_history as existing (
      ...
      canonical_minimum_hp_iv::smallint,
      canonical_minimum_attack_iv::smallint,
      canonical_minimum_defense_iv::smallint,
      1,
      searched_at,
      searched_at
    )
    on conflict (user_id, definition_id) where user_id is not null do update set
      ...
      minimum_hp_iv = excluded.minimum_hp_iv,
      minimum_attack_iv = excluded.minimum_attack_iv,
      minimum_defense_iv = excluded.minimum_defense_iv,
...
returns table (
  ...
  minimum_hp_iv smallint,
  minimum_attack_iv smallint,
  minimum_defense_iv smallint,
...
begin
  if caller_user_id is not null then
    ...
    select definition.target_pal_id,
      ...
      history.minimum_hp_iv,
      history.minimum_attack_iv,
      history.minimum_defense_iv,
...
begin
  ...
  loop
    insert into public.builder_search_history as existing (
      ...
      anonymous_history.minimum_hp_iv,
      anonymous_history.minimum_attack_iv,
      anonymous_history.minimum_defense_iv,
    ...
    on conflict (user_id, definition_id) where user_id is not null do update set
      ...
      minimum_hp_iv = case
        when excluded.last_searched_at >= existing.last_searched_at then excluded.minimum_hp_iv
        else existing.minimum_hp_iv
      end,
      minimum_attack_iv = case
        when excluded.last_searched_at >= existing.last_searched_at then excluded.minimum_attack_iv
        else existing.minimum_attack_iv
      end,
      minimum_defense_iv = case
        when excluded.last_searched_at >= existing.last_searched_at then excluded.minimum_defense_iv
        else existing.minimum_defense_iv
      end,
...
revoke execute on function public.record_builder_search(text, text[], text, integer, text, integer, integer, integer)
  from public, anon, authenticated;
revoke execute on function public.list_recent_builder_searches(text, integer)
  from public, anon, authenticated;
revoke execute on function public.claim_recent_builder_searches(text)
  from public, anon, authenticated;
grant execute on function public.record_builder_search(text, text[], text, integer, text, integer, integer, integer)
  to anon, authenticated;
grant execute on function public.list_recent_builder_searches(text, integer)
  to anon, authenticated;
grant execute on function public.claim_recent_builder_searches(text)
  to authenticated;

comment on column public.builder_search_history.minimum_hp_iv is
  'Optional inclusive 1-100 HP floor requested by the builder.';
comment on column public.builder_search_history.minimum_attack_iv is
  'Optional inclusive 1-100 ranged Attack floor requested by the builder.';
comment on column public.builder_search_history.minimum_defense_iv is
  'Optional inclusive 1-100 Defense floor requested by the builder.';
