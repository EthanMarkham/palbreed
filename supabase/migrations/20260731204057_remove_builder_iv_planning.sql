...
update public.builder_search_history
set objective = 'recommended'
where objective = 'ivs';
...
alter table public.builder_search_history
  ...
  add constraint builder_search_history_objective_check
    check (objective in ('recommended', 'fewest', 'cleanest'));

drop function if exists public.record_builder_search(text, text[], text, integer, text, integer);
drop function if exists public.record_builder_search(text, text[], text, integer, text);
drop function public.list_recent_builder_searches(text, integer);
drop function public.claim_recent_builder_searches(text);

alter table public.builder_search_history
  drop constraint if exists builder_search_history_minimum_iv_check,
  drop column if exists minimum_iv;
