...
Run `npm run data:generate` to refresh the versioned breeding artifacts from the current 1.0 source bundle. Pass a local bundle or direct URL after `--` to regenerate from a specific source. The passive, stat, and save-alias generators require explicit local source files; their argument contracts are printed by `npm run data:passives`, `npm run data:stats`, and `npm run data:save-aliases`.
